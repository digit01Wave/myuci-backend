<html>
<head><title>View Calendar List</title>
<style>
body {
...
<?php
    ...
    $db = new DB_Functions();
    $calendar_events = $db->getAllCalendarEvents();
    if ($calendar_events != false)
        $no_of_events = mysqli_num_rows($calendar_events);
    else
        $no_of_events = 0;
?>
<?php
    if ($no_of_events > 0) {
        ...
        <?php
            while ($row = mysqli_fetch_array($calendar_events)) {
        ?>
        <tr>
